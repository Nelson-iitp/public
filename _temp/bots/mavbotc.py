...
class MAVBOT:
    ...
    # controls          roll,  pitch,   yaw,  altitude
    control_low =  (   -5.0,   -5.0,   -5.0,    1.0  )
    control_high = (    5.0,    5.0,    5.0,    100.0  )
    control_zero = (    0.0,    0.0,    0.0,    1.0  )

    control_dlow =  (   -0.05,   -0.05,   -0.05,    -0.05  )
    control_dhigh = (    0.05,    0.05,    0.05,    0.05  )
    control_dzero = (    0.0,    0.0,    0.0,       0.0  )

    def __init__(self, robot, time_step_multiplier, delta_action=False):
    ...
    def build_spaces(self):
        self.observation_space = gym.spaces.Box(shape=(19,), low=-np.inf, high=np.inf, dtype=self.state_dtype)
        ...
        self.roll_disturbance= self.S[11:12]
        self.pitch_disturbance=self.S[12:13]
        self.yaw_disturbance=self.S[13:14]
        self.target_altitude=self.S[14:15]
        self.actions = self.S[11:15]
        self.FLv, self.FRv, self. RLv, self.RRv = self.S[15:16], self.S[16:17], self.S[17:18], self.S[18:19]
        self.velocities = self.S[15:19]

    
        self.action_shape = (4,) # controls          roll,  pitch,   yaw,  altitude
        self.action_zero = np.array(self.control_zero, dtype=self.action_dtype)
        self.action_space = gym.spaces.Box(
                    shape=   self.action_shape, 
                    low=    -np.ones_like(self.action_zero), 
                    high=    np.ones_like(self.action_zero),  
                    dtype=   self.action_dtype)
        
        self.mapped_drange = \
                (np.array(self.control_dlow, dtype=self.action_dtype), 
                np.array(self.control_dhigh, dtype=self.action_dtype))
        
        self.mapped_range = \
            (np.array(self.control_low, dtype=self.action_dtype), 
                np.array(self.control_high, dtype=self.action_dtype))

        self.action_mapper = REMAP(
            Input_Range=(self.action_space.low, self.action_space.high), 
            Mapped_Range=  (self.mapped_drange if self.delta_action else self.mapped_range))

        
    def _start_motor(self):
        print('Motor Starting...')
        self.actions[:] = self.action_zero
        self.velocities[:] = 1.0
        self._step_motor()
    ...
    def _step_motor(self):
        for m, v in zip(self.motors, self.velocities):
            m.setVelocity(v)
    ...
    def is_terminal(self):
        ...
        if self.gZ<=0.0657594:
            if self.roll < -0.7 or self.roll > 0.7 or self.pitch < -0.7 or self.pitch > 0.7:
                self._stop_motor() #self.led_dLAND.set(1)
                #print('Stopping :: Crashed!!')
                return True #<---- drone has crashed
        return False

    def is_ready(self):
        return(self.gps_read[-1]>=self.target_altitude[0])

    def step(self, action):
        # write actuator inputs
        #print(self.roll, self.rollA, self.pitch, self.pitchA, self.yaw, self.yawA)
        if self.delta_action:
            self.actions[:] += self.action_mapper.in2map(action) # roll pitch, yaw, alt
            self.actions[:] = np.clip(self.actions[:], self.mapped_range[0], self.mapped_range[1])
        else:
            self.actions[:] = self.action_mapper.in2map(action) # roll pitch, yaw, alt
        # Compute the roll, pitch, yaw and vertical inputs --> Actuate the motors taking into consideration all the computed inputs.
        ...
        self.velocities[0] = (vertical_thrust  - roll_input + pitch_input - self.yaw_disturbance) # front left
        self.velocities[1] = -(vertical_thrust  + roll_input + pitch_input + self.yaw_disturbance) # front right
        self.velocities[2] = -(vertical_thrust  - roll_input - pitch_input + self.yaw_disturbance) # rear left
        self.velocities[3] = (vertical_thrust  + roll_input - pitch_input - self.yaw_disturbance) # rear right
        self._step_motor()
    ...
    def episode(self):
        ...
        if self.delta_action:
            zero_action = np.array(self.control_dzero, dtype=self.action_dtype)
        else:
            zero_action = self.action_mapper.map2in( self.action_zero )
        print('Launch MAV')
        ...
        # first we must wait for bot to reach a hovering state
        while not (terminate):
            ts+=1
            # read sensors outputs #<--- implement as observation_space
            state = self.read() #<--- this updates bot.state with updated data / returns a state
            done = self.is_ready()
            #print(f'{ts=}, {done=}, {state=}')
            if not done: # process behavior
                action = zero_action # self.pie.predict(state)
        ...
        print('Ready MAV')
        #self.target_position = np.array([5.0, 5.0, 5.0])
        self.led_dLAND.set(0)
        terminate=False
...
def train_ddpg(bot):
    ...
    if bot.delta_action:
        zero_action = np.array(bot.control_dzero, dtype=bot.action_dtype)
    else:
        zero_action = bot.action_mapper.map2in( bot.action_zero )
    ...
    last_distance_from_target = np.sum(np.abs(bot.location - target_position))
    ...
    ready = False
    while not ready:
        state = bot.read() #<--- this updates bot.state with updated data / returns a state
        done = bot.is_ready()
        #print(f'{ts=}, {done=}, {state=}')
        if not done: # process behavior
            timeout = bot.step(zero_action) # write actuators inputs
        ready = (timeout or done)

    print('Ready MAV')
    last_distance_from_target = np.sum(np.abs(bot.location - target_position))
    bot.led_dLAND.set(0)

    while start_step< start_steps:
        ts+=1 # stepping ... (running timestep)
        start_step+=1
        # observe a state and select action
        state = bot.read() #<--- this updates bot.state with updated data / returns a state
        done = bot.is_terminal()
        #print(f'{ts=}, {done=}, {state=}')
        if not done: # process behavior
            action = action_space.sample()
            timeout = bot.step(action) # write actuators inputs
            distance_from_target =np.sum(np.abs(bot.location - target_position))
            reward = float(distance_from_target - last_distance_from_target)
